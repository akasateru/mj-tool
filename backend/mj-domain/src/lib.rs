...
mod tests {
    ...
    #[test]
    fn child_ron_30fu_4han_is_7700() {
        let fact = Fact {
        ...
        let v = validate_fact(fact).unwrap();
        let r = calc_score(&v, RULESET_V1);
        assert_eq!(r.display_hand_points, "7700");
        assert_eq!(r.totals.hand_points_total_to_winner, 7700);
    }

    #[test]
    fn dealer_tsumo_40fu_3han_is_2600_all() {
        // 40 * 2^(3+2)=1280 base -> 1280*2=2560 -> 2600 each
        let fact = Fact {
            winner_role: WinnerRole::Dealer,
        ...
        let v = validate_fact(fact).unwrap();
        let r = calc_score(&v, RULESET_V1);
        assert_eq!(r.display_hand_points, "2600オール");
        assert_eq!(r.totals.hand_points_total_to_winner, 2600 * 3);
    }

    #[test]
    fn child_ron_40fu_4han_is_mangan_8000() {
        // 40 * 2^(4+2)=2560 base -> cap 2000 -> 2000*4=8000
        let fact = Fact {
            winner_role: WinnerRole::NonDealer,
            win_method: WinMethod::Ron,
            han: 4,
            ...
            honba: 0,
            riichi_sticks: 0,
            discarder: Some(Discarder::Opponent2),
        };
        let v = validate_fact(fact).unwrap();
        let r = calc_score(&v, RULESET_V1);
        assert_eq!(r.display_hand_points, "8000");
        assert_eq!(r.breakdown.limit, LimitKind::Mangan);
    }

    #[test]
    fn han_13_or_more_is_sanbaiman_by_spec() {
        let fact = Fact {
            winner_role: WinnerRole::NonDealer,
            win_method: WinMethod::Ron,
            han: 13,
            ...
            honba: 0,
            riichi_sticks: 0,
            discarder: Some(Discarder::Opponent3),
        };
        let v = validate_fact(fact).unwrap();
        let r = calc_score(&v, RULESET_V1);
        assert_eq!(r.breakdown.limit, LimitKind::Sanbaiman);
        // 三倍満：基礎点6000 → 子ロン 24000
        assert_eq!(r.display_hand_points, "24000");
    }
}
